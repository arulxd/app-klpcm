<x-app-layout>
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <h2 class="text-title-md2 font-bold text-black text-2xl">Master Data Dokter</h2>
    </div>

    <div class="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        
        <form action="{{ route('dokter.index') }}" method="GET" class="flex items-center gap-2">
            @if(request('search'))
        ...
        <div class="flex gap-3 w-full sm:w-auto">
            <form action="{{ route('dokter.index') }}" method="GET" class="relative w-full sm:w-64">
                ...
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari Dokter / Spesialis..." 
            ...
            <a href="{{ route('dokter.create') }}" class="inline-flex shrink-0 items-center gap-2 rounded-xl bg-indigo-600 px-4 py-2 text-sm font-bold text-white hover:bg-indigo-700 transition shadow-md">
    ...
    <div class="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
        <table class="w-full text-left border-collapse">
            <thead>
                <tr class="bg-gray-50/50 border-b border-gray-100 text-xs uppercase text-gray-500 font-bold">
                    ...
                    <th class="px-6 py-4">Spesialis</th>
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50/50 transition">
                    ...
                    <td class="px-6 py-4 font-bold text-gray-900">
                        {{ $item->nama }} 
                        @if($item->kode_dokter)
                            <span class="text-xs text-gray-400 font-normal ml-1">({{ $item->kode_dokter }})</span>
                        @endif
                    </td>
                    ...
                    <td class="px-6 py-4 text-center">
                        <div class="flex justify-center gap-2">
                            <a href="{{ route('dokter.edit', $item->id) }}" class="p-2 text-gray-500 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition" title="Edit">
                                <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                            </a>
                            
                            <form action="{{ route('dokter.destroy', $item->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Yakin ingin menghapus dokter ini?');">
                                @csrf @method('DELETE')
                ...
                @empty
                <tr><td colspan="4" class="px-6 py-8 text-center text-gray-500 italic">Belum ada data dokter.</td></tr>
                @endforelse
            </tbody>
        </table>
        ...
        <div class="px-6 py-4 border-t border-gray-100 bg-gray-50/30 flex flex-col sm:flex-row justify-between items-center gap-4">
            <div class="text-sm text-gray-500">
                Menampilkan <b>{{ $dokter->firstItem() ?? 0 }}</b> s/d <b>{{ $dokter->lastItem() ?? 0 }}</b> dari total <b>{{ $dokter->total() }}</b> data
            </div>
