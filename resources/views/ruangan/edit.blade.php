<x-app-layout>
    <div class="mb-6 flex items-center justify-between">
        <h2 class="text-2xl font-bold text-black">Edit Data Ruangan</h2>
        ...
        <form action="{{ route('ruangan.update', $ruangan->id) }}" method="POST">
            ...
            <div class="grid grid-cols-1 gap-6">
                <div>
                    ...
                    <input type="text" name="kode" value="{{ $ruangan->kode }}" required maxlength="10"
                ...
                <div>
                    ...
                    <div class="relative">
                        <select name="tipe" required class="w-full appearance-none rounded-lg border border-gray-300 bg-white px-4 py-3 pr-8 text-sm focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 outline-none transition">
                            <option value="rawat_inap" {{ $ruangan->tipe == 'rawat_inap' ? 'selected' : '' }}>Rawat Inap</option>
                            <option value="rawat_jalan" {{ $ruangan->tipe == 'rawat_jalan' ? 'selected' : '' }}>Rawat Jalan (Poliklinik)</option>
                            <option value="igd" {{ $ruangan->tipe == 'igd' ? 'selected' : '' }}>IGD (Gawat Darurat)</option>
                            <option value="ok" {{ $ruangan->tipe == 'ok' ? 'selected' : '' }}>Kamar Operasi</option>
                            <option value="penunjang" {{ $ruangan->tipe == 'penunjang' ? 'selected' : '' }}>Penunjang (Lab/Rad)</option>
                        </select>
